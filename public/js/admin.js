    ...
    inputBuscador.addEventListener('input', function(event) {
        ...
        identificadorTemporizador  = setTimeout(function() {
                ...
                if (valorInputSearch != '') {                   // si el valor el input es diferente a vacio
                    ...
                    window.location.href = `/admin/?buscar=${encodeURIComponent(valorInputSearch)}`;    // Entonces hubo una busqueda y redirigimos a la siguiente ruta
                
                }else{                                          // sino

                    window.location.href = `/admin/`;           // entonces el input esta vacio y debemos mostrar todos los productos
